...
convert() {
  ...
  <"$FILE" yq '(select(.kind == "Secret" and .type == "kubernetes.io/tls") | .data) = {}' |
    ...
    yq '(select(.metadata.namespace == "monitor" and .kind == "Secret") | .data) = {}' |
    yq '(select(.metadata.namespace == "monitor" and .kind == "Deployment") | .spec.template.metadata.annotations.checksum/secret) = ""' |
